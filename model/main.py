...
def parse_args():
    ...
    parser.add_argument('--step-size', type=int, default=30, help='Specify step size for LR scheduler')
    parser.add_argument('--gamma', type=float, default=0.1, help='Specify decay factor for LR scheduler')
...
def get_config():
    ...
    for dataset_type in args.dataset_types:
        assert dataset_type in [i.name for i in DatasetTypes], f"{dataset_type} is not a valid dataset type"
    ...
    assert args.device in ["cpu", "mps", "cuda"] + [f"cuda:{n}" for n in range(8)], "Specified device must be either cpu, cuda, or mps!"
    assert args.model_type == 1 or args.model_type == 2, "Model type must either be EfficientNet(1) or ResNet(2)!"
...
def train_model(config, plot_data=False):    
    ...
    train_dataset = Data(config.extract_lsb, converted_dataset_types, filepath=os.path.join("data", "train"), mode="train")
    ...
    criterion = nn.CrossEntropyLoss(weight=[sum(train_dataset.dataset_sizes)/i for i in train_dataset.dataset_sizes])
